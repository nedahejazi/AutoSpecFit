...
An additional fitted synthesis parameter in this configuration is:
...
def run_systematic_abundance_error_analysis(
    ...
    corresponding physical [X/H] scale. Because the requested +/-1-sigma value
    may not lie exactly on the synthesis grid, each measured abundance shift is
    ...
    systematic error is added in quadrature to the chi-square random abundance
    error.
    ...
    for parameter_name in parameter_names:
        ...
        # Each entry is an absolute abundance response rescaled from the actual
        # grid displacement to the requested 1-sigma parameter displacement.
        ...
        for sign, sign_label in ((+1.0, "plus"), (-1.0, "minus")):
            ...
            # If sigma is smaller than half a grid step, nearest-grid rounding
            # returns the nominal point and would incorrectly imply zero
            # sensitivity. Evaluate the immediately adjacent grid point in the
            # requested direction instead, then rescale that response to sigma.
            ...
            # At a hard grid boundary a requested side may still have no
            # distinct model point. In that case it is omitted and the other
            # available side supplies the one-sided response.
            if (
            ...
            # The synthesis was evaluated at ``actual_displacement``, which can
            # differ from sigma after rounding to the model grid. Under a local
            # linear-response approximation, convert that measured response to
            # the abundance shift corresponding to exactly one reported sigma.
        ...
        if scaled_shifts:
            ...
            # Average the available absolute +sigma and -sigma responses for
            # each element. At a grid boundary, or when one fitted abundance is
            # non-finite, use the single available side rather than treating the
    ...
    with open(log_path, log_mode, buffering=1) as log_handle:
        ...
        while iteration_id <= n_total_iterations:
            ...
            if next_stage == "parameter":
                ...
                # Abundance iteration N refines the atmosphere that will be
